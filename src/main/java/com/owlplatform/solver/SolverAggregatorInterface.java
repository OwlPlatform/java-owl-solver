...
public class SolverAggregatorInterface {
	...
	private static final class AdapterHandler implements SolverIoAdapter {
		...
		public void subscriptionRequestReceived(IoSession session,
			...
			this.parent.subscriptionResponseReceived(session,
					subscriptionMessage);
		}
		...
		@Override
		public void solverSampleSent(IoSession session,
	...
	/**
	 * The object that will pass events to this SolverAggregatorInterface. Used
	 * to hide interface methods.
	 */
	...
	/**
	 * Returns the set of rules that have been or will be sent to the
	 * aggregator.
	 * 
	 * @return the rules
	 */
	...
	/**
	 * Sets the new rules to send to the aggregator. Changes to the rules will
	 * not take effect until the next time a connection is made. If the new rule
	 * set is intended to replace the current set completely, then the caller
	 * should disconnect and connect to the aggregator.
	 * 
	 * @param rules
	 *            the rules to set
	 */
	...
	/**
	 * Creates a new SolverAggregatorConnection with the provided
	 * SolverIoHandler as the IO handler.
	 * 
	 * @param ioHandler
	 ...
	 * after calling this method. This method has been replaced by
	 * {@link #connect(long)}, and is equivalent to calling {@code #connect(0)}.
	 * 
	 ...
	 * Initiates a connection to the Aggregator (if it is not yet connected). If
	 * {@code #stayConnected} is {@code true}, then this method will NOT return
	 * until a connection is established or the timeout is exceeded. A timeout
	 * of 0 indicates an infinite timeout value.
	 * 
	 * @param maxWait
	...
	public boolean connect(long maxWait) {
		...
		do {
			...
			if (this.stayConnected) {
				...
				try {
					log.warn(String
							.format("Connection to %s:%d failed, waiting %dms before retrying.",
	...
	/**
	 * Clean-up any remaining connection-related state. Notify listeners that
	...
	/**
	 * Shuts down this connection to the aggregator. This method has been
	 * replaced by {@link #disconnect()}.
	 */
	@Deprecated
	 ...
	 * before giving-up, or an infinite amount of time if {@code timeout} is
	 * &le; 0.
	 * 
	 * @param timeout
	...
	protected void _disconnect() {
		...
		if (currentSession != null) {
			if (!currentSession.isClosing()) {
				...
				log.info(
						"Closing connection to aggregator at {} (waiting {}ms).",
	 ...
	 *            the sent sample.
	 */
	protected void solverSampleSent(IoSession session,
			SampleMessage sampleMessage) {
	...
	/**
	 * Sets the host for the aggregator to connect to. Changes to this value
	 * don't have any effect until a connection attempt is made to the
	 * aggregator.
	 * 
	 * @param host
	 ...
	 * 
	 * @return {@code Boolean.TRUE} if both handshakes (sent/received) are
	 *         valid, {@code Boolean.FALSE} if either is invalid, and
	...
	/**
	 * Sets the new value for the connection timeout. Changes to this value
	 * don't have any effect until a connection attempt is made.
	 * 
	...
	/**
	 * Sets whether or not the interface should reconnect to the aggregator
	 ...
	 *            {@code true} if this interface should disconnect when an
	 *            exception is thrown, or {@code false} if it should maintain
	 *            the connection.
	 */
